# Auto Handwrap Roller v1
복싱·킥복싱 등  격투기에서 사용하는 핸드랩을 자동으로 말아주는 전동 롤러

## 1. 개요 
Auto Handwrap Roller는 복싱, 킥복싱, MMA 등 격투기에서 사용하는 핸드랩을 자동으로 말아주는 장치이다. 
...
마지막으로 포멕스는 보드와 모터를 넣을 케이스를 만들기 위해 사용하였다. 포멕스 대신 다른 재료(목재, 아크릴 등등)를 사용해도 무방하다.
...
#### 2.1.3. 소스코드 
Arduino IDE에서 <strong style="color:blue">'src/autoHandwrapRoller.ino'</strong> 파일을 열고 상단 메뉴의 Tools 에서 아래와 같이 설정한다.
|항목|값|
|-----|---|
|Board|Arduino Nano|
